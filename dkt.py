...
def main(dataset, split_file, which=0, hidden_units=200, batch_size=5, time_window=100, epochs=50):
    '''
    ...
    addin = os.path.split(split_file)[1].replace(".txt",'').replace(os.path.split(dataset)[1].replace(".txt",""), '').replace('_split', '-')
    ...
    training_seqs, tests, num_skills = load_dataset(dataset, split_file)
    if len(tests[which]) < 1:
        return

    testing_seqs = deepcopy(tests[which])

    model_file = dataset + str(addin) + '.model_weights'
    history_file = dataset + str(addin) + '.history'
    preds_file = dataset + str(addin) + '.preds'

    print("Training Sequences: %d" % len(training_seqs))
    print("Testing Sequences: %d" % len(testing_seqs))
    #print("Testing Sequences: %d" % len(tests))
    ...
    def loss_function(y_true, y_pred):
        ...
        rel_pred = tf.keras.backend.sum(y_pred * skill,axis=2)
    ...
    def finished_batch(percent_done):
        print
        "(%4.3f %%) %f" % (percent_done, overall_loss[0])
        model.reset_states()
...
def load_dataset(dataset, split_file):
    ...
    tests = {j:[seqs[i] for i in range(0, len(seqs)) if student_assignment[i] == str(j)] for j in range(20)}
    #testing_seqs = [seqs[i] for i in range(0, len(seqs)) if student_assignment[i] == '0']

    #return training_seqs, testing_seqs, num_skills
    return training_seqs, tests, num_skills
...
if __name__ == "__main__":
    #base = '/Users/ahill6/PycharmProjects/dkt/data'
    base = '/Users/ahill6/PycharmProjects/dkt/AdvML'
    data = base + 'assistments.txt'
    splt = base + 'split.txt'
    go = True

    #xx = ['Assistments','MACFRENCH','MACSPANISH','USNA']
    for d in [f for f in os.listdir(base) if 'err.txt' not in f if 'DS_Store' not in f if 'split' not in f]:
        #for d in [f for f in os.listdir(base) if 'err.txt' not in f if 'DS_Store' not in f if
        #          not any([x in f for x in xx])]:
        print(d)
        '''
        if 'USNAFA05' not in d:
            exit()
        if not go:
            if 'USNAFA05' in d:
                go = True
        ...
        '''
        try:
            #err = open(os.path.join(base,d,"err.txt"),'w')
            #data = os.path.join(base, d, d[:-1] + ".txt")
            #splt = os.path.join(base, d, d[:-1] + "_split.txt")
            err = open(os.path.join(base,d.replace(".txt","")+"err.txt"),'w')
            data = os.path.join(base, d)
            for i in range(10):
                splt = os.path.join(base, d.replace(".txt","_split")+str(i)+".txt")
                #main(data, splt,which=i,epochs=3)
                print(data,splt)
                main(data, splt, epochs=30)
            sleep(30)
            err.close()
        ...
        for i in range(9):
            print(i)
            try:
                data = os.path.join(base,d,str(i)+".txt")
                splt = os.path.join(base,d,str(i)+"assistments_split.txt")
                main(data, splt)
            except Exception as e:
                err.write(str(e)+"\n")
            if 'Assistments' in d and i < 4:
                continue
            sleep(15)
        err.close()
        '''
